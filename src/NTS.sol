...
/**
 * Evo! Ovo je plod mog razmišljanja kako učiniti nešto novo i zabavno na blockchainu, a opet da ima inovativnu "tokenomiju", jer ljudi jako pate na to :) Kad malo bolje pogledam, nije contract baš ni jednostavan, pa ću pokušati što sažetije objasniti što radi:

Zamislio sam svojevrsnu "igru rata" na blockchainu između korisnika i pametnog ugovora (zvanog NTS, iliti "nuke the supply"), gdje pametni ugovor ima zadatak prodati supply ICBM tokena kroz 365 dana, a korisnici će koristiti funkcije namijenjene da to spriječe.

Contract minta dva coina, ICBM (ICBM) i WH (Warhead). ICBM supply je 830M coinova, nakon deploya, 100M ide kreatoru, 730M ostaje u NTS ugovoru. Također, minta se 1000 WH tokena, samo da bi se mogao napraviti trading par. Nakon deploya, ja bi ručno napravio ICBM/WETH i WH/WETH uniswap v3 parove.
Kako sam rekao, dnevna količina za prodaju je defaultSell (830M / 365 dana), 2M dnevno.

Od trenutka deploya, počinje "preparation phase" i traje 48 sati. Za to vrijeme contract ne odrađuje dnevne prodaje, ali korisnici mogu koristiti svoje funkcije, "arm" i "nuke". nakon "preparation phase"-a, počinje "operations phase" i traje 365 dana (to su tih 365 dana gdje NTS contract prodaje preostali supply svaki dan.

ovo su tri osnovne funkcije:
- funkcija "arm" odabranu količinu ICBM tokena od korisnika šalje u contract na minimalno 24 sata, (naoružavanje raketa) - tokom tog vremena korisnici ne mogu do tih tokena. 
- funkcija "nuke" - da bi korisnici dobili svoje ICBM tokene natrag, pokreću "nuke" funkciju, kojom uništavaju dio supply-a ICBM-a koju drži NTS contract i za to bivaju nagrađeni Warhead tokenom. odnos ICBM iskorišteni za "nukiranje" i ICBM uništeni u NTS contractu je 10:1, a nagrada u Warhead tokenu je 1:1, po jedan Warhead token kojeg NTS contract minta i šalje korisniku, za jedan uništeni ICBM token iz preostalog supply-a.

 funkcija "sell": ovu funkciju sam zamislio kao automatsku, da ju ChainLink keeper izvodi ali sam od toga odustao pa sam odlučio da ju pokreću korisnici i za to bivaju nagrađeni svježe mintanim WH tokenom
NTS contract prvo računa količinu za prodati tog dana i koristi formulu:

dailySell = (defaultSell * day) - (totalSold + totalBurned)

totalSold i totalBurned varijable predstavljaju sveukupnu količinu koju je NTS contract prodao od početka i sveukupnu količinu koju su korisnici "iznukirali" od početka. Ta formula, omogućava da NTS contract uopće ni ne proda ništa, ako su korisnici bili vrijedni i marljivo nukirali protekli dan, ukoliko dailySell bude 0 ili manji od nule.
...
I na kraju, za nagradu onome tko je izvršio "sell" funkciju (makar ne bilo prodaje ICBM-a), NTS bi izmintao i poslao Warhead token u količini 0.001% trenutnog Warhead supply-a kao kompenzaciju.

Eto.. tako sam ja to nekako zamislio, kao igru na blockchainu, NTS ugovor želi prodati ICBM supply, korisnici to žele spriječiti, budu li dobri, dobiju Warhead za nagradu. Ne budu li uspjevali, ugovor će pumpati cijenu Warheada pa da daobiju warhead, morat će nukirati... Kreirao sam nekakvu push-pull mehaniku između ICBM i Warhead tokena izgleda, možda ljudima bude zanimljivo. Oće bit šta od ovog?
